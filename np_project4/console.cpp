...
void console::init_clients()
{
    ...
    if ( index < (int)vtr.size() && vtr[index].length() != 0 )
    {
        SocksIP = vtr[index].substr(3);
        // std::cout<<"SocksIP is "<<SocksIP<<std::endl;
    }
    index++;
    if ( index < (int)vtr.size() && vtr[index].length() != 0 )
    {
        SocksPort = vtr[index].substr(3);
        // std::cout<<"SocksPort is "<<SocksPort<<std::endl;
    }
}

void console::connect_SocksServer()
{
    for ( int i=0; i<(int)clients.size(); i++ )
    {
		...
		int temp = clients[i].server_port / 256;
		sock_request += (char)(temp > 128 ? temp - 256 : temp);
		temp = clients[i].server_port % 256;
		sock_request += (char)(temp > 128 ? temp - 256 : temp);
		...
		sock_request += clients[i].server_addr;
		sock_request += (char)0;       
        
        boost::asio::ip::tcp::resolver resolver(g_io_context);
        boost::asio::ip::tcp::resolver::query query(SocksIP, SocksPort);
...
void console::get_sock_reply(int session)
{
    clients[session].socket.async_read_some((boost::asio::buffer(clients[session].data,8)),
    [this, session](boost::system::error_code ec, std::size_t length)
    {
        if ( ec || clients[session].data[0] != 0)
        {
            clients[session].socket.close();
            return;
        }

        std::vector<std::string> input = get_shell_input(clients[session].testfile);
